...
func TestTraceError(t *testing.T) {
	...
	line, isPrefix, err = br.ReadLine()
	assert.Equal(t, "github.com/cloudernative/go-zero/core/errs/errs_test.grandson", string(line))
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)

	_, _, _ = br.ReadLine()
	line, isPrefix, err = br.ReadLine()
	assert.Equal(t, "github.com/cloudernative/go-zero/core/errs/errs_test.child", string(line))
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)

	_, _, _ = br.ReadLine()
	line, isPrefix, err = br.ReadLine()
	assert.Equal(t, "github.com/cloudernative/go-zero/core/errs/errs_test.parent", string(line))
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)
}
...
func TestTraceErrorSetStackSkip(t *testing.T) {
	...
	line, isPrefix, err = br.ReadLine()
	t.Log(string(line))
	assert.Contains(t, string(line), "github.com/cloudernative/go-zero/core/errs/errs_test.TestTraceErrorSetStackSkip")
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)

	_, _, _ = br.ReadLine()
	line, isPrefix, err = br.ReadLine()
	assert.Equal(t, "github.com/cloudernative/go-zero/core/errs/errs_test.TestTraceErrorSetStackSkip.func1", string(line))
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)

	_, _, _ = br.ReadLine()
	line, isPrefix, err = br.ReadLine()
	assert.Equal(t, "github.com/cloudernative/go-zero/core/errs/errs_test.TestTraceErrorSetStackSkip", string(line))
	assert.Equal(t, isPrefix, false)
	assert.Nil(t, err)
}
...
func TestErrorChain(t *testing.T) {
	var e error = errs.Wrap(os.ErrDeadlineExceeded, int(errs.TrpcRetCode_TRPC_CLIENT_INVOKE_TIMEOUT_ERR), "just wrap")
	require.Contains(t, errs.Msg(e), os.ErrDeadlineExceeded.Error())
	e = fmt.Errorf("%w", e)
	require.Equal(t, errs.TrpcRetCode_TRPC_CLIENT_INVOKE_TIMEOUT_ERR, errs.Code(e))
...
func TestIsTimeout(t *testing.T) {
	require.True(t, (&errs.Error{
		...
		Code: errs.TrpcRetCode_TRPC_CLIENT_INVOKE_TIMEOUT_ERR,
	...
	require.True(t, (&errs.Error{
		Type: errs.ErrorTypeFramework,
		Code: errs.TrpcRetCode_TRPC_CLIENT_FULL_LINK_TIMEOUT_ERR,
	}).IsTimeout(errs.ErrorTypeFramework))
	require.True(t, (&errs.Error{
		Type: errs.ErrorTypeFramework,
		Code: errs.TrpcRetCode_TRPC_SERVER_TIMEOUT_ERR,
	}).IsTimeout(errs.ErrorTypeFramework))
	require.True(t, (&errs.Error{
		Type: errs.ErrorTypeFramework,
		Code: errs.TrpcRetCode_TRPC_SERVER_FULL_LINK_TIMEOUT_ERR,
	}).IsTimeout(errs.ErrorTypeFramework))
	require.False(t, (&errs.Error{
		Type: errs.ErrorTypeFramework,
